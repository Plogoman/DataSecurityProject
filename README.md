...
> **DES is cryptographically broken and must not be used to protect real data.**
>
> The 56-bit effective key length makes DES vulnerable to exhaustive brute-force search. DES was publicly demonstrated as breakable in under 24 hours by the EFF's Deep Crack machine in 1998, and NIST formally withdrew the standard in 2005. Triple-DES (3DES) extended its life but has since been deprecated as well.
>
> This project exists solely as an educational exercise to understand the internals of a historical symmetric block cipher. For any real-world cryptographic need, use a modern, well-audited algorithm such as **AES-256-GCM** via a vetted library (OpenSSL, libsodium, Bouncy Castle).
